...
with
    static member None = Seq.empty<OptionalParameter>
...
/// An object that contains an access token for the DeviantArt API and can fetch an additional token if the main token is invalid.
type IDeviantArtRefreshableAccessToken =
    inherit IDeviantArtAccessToken

    /// Updates the access token, or does nothing if no new token is available.
    abstract member RefreshAccessTokenAsync: CancellationToken -> Task
