...
module Update =
    let AsyncExecute token common (stackid: int64) (updates: UpdateField seq) = async {
        let query = seq {
    ...
    let ExecuteAsync token common stackid updates =
        AsyncExecute token common stackid updates
        |> Async.StartAsTask