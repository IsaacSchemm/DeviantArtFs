...
let sandbox token_string = async {
    ...
    let! m =
        DeviantArtFs.Requests.Messages.FeedbackMessageType.Activity
        |> DeviantArtFs.Requests.Messages.FeedbackMessagesRequest
        |> DeviantArtFs.Requests.Messages.FeedbackMessages.AsyncExecute token (page 0 10)
    printfn "%A" m
}
