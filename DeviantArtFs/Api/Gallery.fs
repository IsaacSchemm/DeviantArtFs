...
module Gallery =
    ...
    let GetAllViewAsync token scope batchsize offset = Utils.buildAsyncSeq {
        get_page = (fun offset -> AsyncPageAllView token scope batchsize offset)
    ...
    let AsyncPageGallery token scope folder limit offset =
        let folder_id_str =
            match folder with
    ...
    let GetGalleryAsync token scope folder batchsize offset = Utils.buildAsyncSeq {
        get_page = (fun offset -> AsyncPageGallery token scope folder batchsize offset)
    ...
    let AsyncCreateFolder token folder description parent_folderid =
        seq {
            yield "folder", folder
    ...
    let AsyncMoveFolder token folderid to_folderid =
        seq {
